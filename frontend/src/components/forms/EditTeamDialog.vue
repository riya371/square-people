...
function fill() {
  ...
  leadEmployeeId.value = t.leadEmployeeId ? String(t.leadEmployeeId) : ''
  error.value = ''
...
watch(() => props.team, () => { if (props.open) fill() })
...
async function submit() {
  ...
  try {
    const payload = {
      ...
      leadEmployeeId: leadEmployeeId.value ? Number(leadEmployeeId.value) : null,
    }
    await teamsStore.update(props.team.id, payload)
    emit('update:open', false)
    emit('saved')
  } catch (err) {
    error.value = err?.response?.data?.error?.message || err.message || 'Failed to update team'
  } finally {
    submitting.value = false
  }
}
...
<template>
  <Dialog :open="open" title="Edit team" @update:open="(v) => emit('update:open', v)">
