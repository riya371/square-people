...
async function submit() {
  ...
  try {
    ...
    await departmentsStore.update(props.department.id, payload)
    emit('update:open', false)
    emit('saved')
  } catch (err) {
    error.value = err?.response?.data?.error?.message || err.message || 'Failed to update department'
  } finally {
    submitting.value = false
  }
}
...
<template>
  <Dialog :open="open" title="Edit department" @update:open="(v) => emit('update:open', v)">
